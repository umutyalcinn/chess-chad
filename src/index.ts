...
function renderBoard(ctx: CanvasRenderingContext2D, state: GameState){
    ...
    for(let i = 0; i < state.pieces.length; i++){
        ...
        image.onload = function() {
            if(piece.position?.y === undefined) return;
            ctx.drawImage(image, CELL_WIDTH * piece?.position.x, CELL_HEIGHT * piece?.position.y, CELL_WIDTH, CELL_HEIGHT)
        };
    }
}

function getPieceByPosition(state: GameState, position: Position) : Piece | null{
    for(let i = 0; i < state.pieces.length; i++){
...
function validMoves(state: GameState, piece: Piece): Move[]{
    if(piece.position == null){
        throw new Error("Moving captured piece!");
    }
    let moves: Move[] = [];
    ...
    switch(piece.type){
        ...
        case "bishop": {
            ...
            for(let i = 1; i < max(CELL_COUNT_X, CELL_COUNT_Y); ++i){
                let moveA: Move = {xOffset: i, yOffset:i };
                let moveB: Move = {xOffset: -i, yOffset:i };
                let moveC: Move = {xOffset: i, yOffset:-i };
                let moveD: Move = {xOffset: -i, yOffset:-i };
...
function positionAfterMove(piece: Piece, move: Move): Position{
    if(piece.position == null){
        throw new Error("Moving captured piece!");
    }

    let newPosition: Position = {x: piece.position.x + move.xOffset, y: piece.position.y + move.yOffset};
...
function isMoveInBorders(piece: Piece, move: Move){
    if(piece.position == null){
        throw new Error("Moving captured piece!");
    }
